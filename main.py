#!/usr/bin/env python3
"""Application entry point for discovery, stable camera slots, and monitoring."""
...
class _RescanBridge(QtCore.QObject):
    """Deliver executor results to the Qt GUI thread.

    Future callbacks run in the executor thread, where ``QTimer.singleShot``
    has no event loop to service it. Emitting this signal instead gives Qt a
    queued delivery to the GUI-owned receiver.
    """
...
def _make_rescan_done_callback(bridge: _RescanBridge) -> Any:
    """Convert a rescan future into a GUI-thread bridge notification.

    A failed probe batch becomes an empty result so the GUI callback still
    clears the in-flight state and leaves the next scan eligible to run.
    """
...
def _plan_rescan_attachments(
    ...
    """Plan a rescan batch and update port bookkeeping in place.

    Successful assignments reserve their slots immediately so one batch
    cannot double-book a placeholder. Failed probes enter cooldown, while a
    working camera blocked only by another port's pin remains retryable.
    ``choose_slot_for_identity`` supplies the pin, previous-slot, and
    lowest-free-slot precedence.
    """
    ...
    for identity, resolved_index in results:
        ...
        if slot is None:
            # Slot reservations are availability constraints, not probe failures.
            continue
...
def _run_rescan_tests(
    ...
    """Resolve each candidate to a working capture node off the GUI thread.

    The provisional stream target is the fast path. A failed by-path target
    expands to the remaining nodes that share its USB port path because the
    lowest node may expose metadata rather than frames. Numeric fallback
    identities have no by-path group to expand.
    """
    results: list[tuple[CameraIdentity, Optional[int]]] = []
...
def _plan_detach_sweep(
    ...
    """Select active camera tiles that must return to placeholder state.

    This decision stays independent of current placeholder availability. A
    full grid still needs the sweep to recover a slot from a worker that has
    exhausted its restart budget or could not be stopped safely.
    """
    return [
...
def _step_widget_fps(w: CameraWidget, stress: bool, profile_ui_fps: int) -> bool:
    """Step one tile's software-emission and UI targets for stress control.

    Stress lowers both targets; recovery restores them toward their baselines.
    Return whether either target changed.
    """
    changed = False
    ...
    if stress:
        ...
        if new_fps > cur:
            w.set_dynamic_fps(new_fps)
            changed = True
        # Recover toward this tile's initial render rate; the profile is a fallback.
...
def safe_cleanup(widgets: list[CameraWidget], cleaned_flag: list[bool]) -> None:
    """Clean each supplied camera tile at most once per application lifetime."""
    if cleaned_flag[0]:
        return
    cleaned_flag[0] = True
    ...
    for w in list(widgets):
        try:
            ...
            # One broken tile must not prevent the remaining workers from closing.
            pass
...
# QApplication.quit() is ineffective before the event loop starts, so retain
# early signal requests for the startup checkpoints.
_shutdown_requested = {"flag": False}


def _handle_shutdown_signal(sig: int, frame: Optional[Any]) -> None:
    """Record a signal shutdown and ask an active Qt loop to exit cleanly."""
    _shutdown_requested["flag"] = True
    QtWidgets.QApplication.quit()


def _startup_shutdown_check() -> None:
    """Honor a shutdown signal once the Qt event loop can process ``quit``.

    Scheduling this as the loop's first event closes the race between a
    pre-exec flag check and the point at which Qt starts honoring quit calls.
    """
...
def _install_signal_handlers(app: QtWidgets.QApplication) -> None:
    """Route terminal and service shutdown signals through Qt cleanup."""
    # The parameter makes the required QApplication lifetime explicit; quit()
    # resolves the active application through Qt's singleton.
    del app
...
def main() -> None:
    """Build the dashboard and run its capture, rescan, and Qt lifecycles."""
    ...
    # Some window managers ignore fullscreen requests before the window is visible.
    mw.show()
    ...
    # Honor signals received during blocking discovery before workers are spawned.
    if _shutdown_requested["flag"]:
    ...
    # Bookkeeping uses stable USB port paths when available; cameras without
    # by-path identity use their synthetic ``index:N`` fallback keys.
    active_ports: set[str] = set()
    failed_ports: dict[str, float] = {}
    last_slot_by_port: dict[str, int] = {}
    port_of_widget: dict[CameraWidget, str] = {}
    ...
    def restart_app():
        """Restart the process after best-effort camera-tile cleanup."""
        ...
        # Cleanup may leave camera descriptors open. Mark matching descriptors
        # so a successful exec closes rather than inherits them under the same PID.
    ...
    def toggle_night_mode():
        """Update shared night-mode state and every current tile."""
    ...
    def set_brightness_all(value: int):
        """Apply a settings-tile brightness choice to every current tile."""
    ...
    # The first grid cell is controls-only and never joins the capture lifecycle.
    ...
    # Keep slot objects stable; hot-plugging changes a tile's role, not the grid.
    ...
    # Consecutive samples debounce brief load and temperature spikes.
    if config.DYNAMIC_FPS_ENABLED:
        stress_counter = {"stress": 0, "recover": 0}

        def adjust_fps():
            """Adapt rates after the configured stress or recovery hold period."""
    ...
    # Run one hardware-probe batch at a time; the bridge returns results to Qt.
    rescan_timer = None
    ...
    def _apply_rescan_results(
        ...
        # The planner updates port and slot bookkeeping before Qt attaches tiles.
        ...
        for identity, slot_index in attachments:
            ...
            if widget is None:
                # Both structures share one placeholder snapshot. A miss means
                # the unique-slot invariant broke after bookkeeping was updated.
                continue
    ...
    # GUI ownership makes executor-thread emissions arrive as queued calls.
    rescan_bridge = _RescanBridge(mw)
    rescan_bridge.results_ready.connect(_apply_rescan_results)

    def rescan_and_attach():
        """Recycle failed slots, then probe cameras eligible to fill them.

        This runs even with a full grid because the detach sweep creates the
        placeholder needed after a camera tile exhausts in-place recovery.
        """
        now = time.time()
        for w in _plan_detach_sweep(list(camera_widgets), now):
            ...
            if detached_idx is not None:
                ...
                # Retain slot memory so this physical camera returns here.
        ...
        now = time.time()
        # A fresh snapshot picks up index changes for a replugged physical port.
    ...
    # The timer owns both detachment and attachment, even when the grid is full.
    rescan_timer.start()
    ...
    # The in-loop checkpoint observes signals received in the pre-loop gap.
